...
scaled_expr <- GetAssayData(pbmc3k, slot = "scale.data")
...
Based on canonical markers, we can assign cell types to clusters.
...
pbmc3k$celltype <- new_cluster_ids[as.character(pbmc3k$seurat_clusters)]

# Reorder for plotting
pbmc3k$celltype <- factor(pbmc3k$celltype,
                          levels = c("Naive CD4 T", "Memory CD4 T", "CD8 T",
                                     "B", "NK", "CD14+ Mono", "FCGR3A+ Mono",
                                     "DC", "Platelet"))
```
...
# SEURAT WRAPPER VERSION
p1 <- DimPlot(pbmc3k, reduction = "umap", group.by = "celltype", label = TRUE, repel = TRUE) +
  labs(title = "Seurat wrapper") +
  NoLegend()
...
umap_celltype$celltype <- pbmc3k$celltype
