...
ggplot2 orders the boxes or the bars according to the factor levels.

This means we can the order them to what we want as long as we change the factor levels
to what we want.

Let me show you how to do that manually without using `fct_reorder`.

```{r}
# we create a new vector of the study according to the median level of EPCAM
tcga_new_factor<- tcga_cancer %>%
  group_by(study) %>%
  summarise(median_EPCAM = median(EPCAM)) %>%
  arrange(desc(median_EPCAM))

head(tcga_new_factor)

# this is the new order we need
# tcga_new_factor$study 

# change the study to the new levels according to our new vector
tcga_cancer$study<- factor(tcga_cancer$study, levels = tcga_new_factor$study)
